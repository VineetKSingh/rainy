...
func (b cfnBuilder) Template(config map[string]string, params map[string]string) (cft.Template, error) {
	defer func() {
		if r := recover(); r != nil {
	...
	for name, ptype := range params {
		prm := b.Parameters[ptype]
		parameters[name] = prm
	}
